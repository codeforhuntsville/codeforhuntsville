...
function send(request, response) {
  ...
  request.on('end', function () {
    ...
    if (messObj.email != null) {
      ...
      var subj = messObj.name + " wants to join Code4Huntsville "
      txt = messObj.txt + messObj.name + " at " + messObj.email + " wants to be a member: \n";
      txt = txt.concat("Message: " + messObj.message);
	  console.log("Body of message: " + txt);
      sendReq.send(messObj.recip, subj, txt, response);
	  //response.writeHead(200, { 'Connection': 'close' });
      //response.end("That's all folks!");
    } else {console.log('invalid message = ' + messObj.message);	}    		
  });
}
